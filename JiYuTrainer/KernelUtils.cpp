	...
	if (XDriverLoaded())
	{
		...
		if (DeviceIoControl(hKDrv, CTL_KILL_PROCESS, &pidb, sizeof(ULONG_PTR), &status, sizeof(status), &ReturnLength, NULL))
		{
			if (status == STATUS_SUCCESS)
				return TRUE;
			else currentLogger->LogError(L"CTL_KILL_PROCESS 错误：0x08%X", status);
		}
...
BOOL KFBeforeUnInitDriver()
{
	if (XDriverLoaded())
	{
		...
		if (DeviceIoControl(hKDrv, CTL_UNINIT, NULL, NULL, NULL, NULL, &ReturnLength, NULL))
			return TRUE;
		else currentLogger->LogError(L"DeviceIoControl CTL_UNINIT 错误：%d", GetLastError());
	}
	return false;
}
BOOL KFInstallSelfProtect()
{
	if (XDriverLoaded())
	{
		...
		if (DeviceIoControl(hKDrv, CTL_INITSELFPROTECT, &pidb, sizeof(pidb), NULL, NULL, &ReturnLength, NULL))
				return TRUE;
		else currentLogger->LogError(L"DeviceIoControl CTL_INITSELFPROTECT 错误：%d", GetLastError());
	}
	return false;
}
