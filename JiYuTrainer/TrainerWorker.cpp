...
bool TrainerWorkerInternal::Kill(bool autoWork)
{
	...
	if (_StudentMainControlled){
		...
		if(!vkill){
			...
			mbp.lpszCaption = L"i.chaoxing - 提示";
	...
	if (NT_SUCCESS(status)) {
		...
		if (status == STATUS_ACCESS_DENIED) goto FORCEKILL;
			...
			if (!autoWork)
				MessageBox(hWndMain, L"无法结束极域电子教室，您需要使用其他工具手动结束", L"i.chaoxing - 错误", MB_ICONERROR);;
			CloseHandle(hProcess);
			return false;
		}
		else if (status == STATUS_INVALID_CID || status == STATUS_INVALID_HANDLE) {
...
FORCEKILL:
	if (XDriverLoaded() && MessageBox(hWndMain, L"普通无法结束极域，是否调用驱动结束极域？\n（驱动可能不稳定，请慎用。您也可以使用 PCHunter 等安全软件进行强杀）", L"i.chaoxing - 提示", MB_ICONEXCLAMATION | MB_YESNO | MB_TOPMOST) == IDYES)
	{
