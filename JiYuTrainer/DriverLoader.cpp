...
BOOL MUnLoadDriverServiceWithMessage(const wchar_t* szSvrName)
{
	...
	if (hServiceMgr == NULL)
	{
		...
		MessageBox(NULL, str, L"i.chaoxing - 错误", MB_ICONERROR);
		bRet = FALSE;
		goto BeforeLeave;
	}
	//打开驱动所对应的服务
	...
	if (hServiceDDK == NULL)
	{
		...
		if (lastErr == ERROR_SERVICE_DOES_NOT_EXIST) 
			MessageBox(NULL, L"驱动已卸载并删除，请不要重复操作", L"i.chaoxing - 提示", MB_ICONEXCLAMATION);
		else if ( lastErr == ERROR_SERVICE_MARKED_FOR_DELETE) 
			MessageBox(NULL, L"没有在这台计算机上找到找到驱动，可能是驱动已经被卸载了", L"i.chaoxing - 提示", MB_ICONEXCLAMATION);
		else {
